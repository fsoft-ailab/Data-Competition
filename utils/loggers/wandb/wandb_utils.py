...
def check_wandb_config_file(data_config_file):
    wandb_config = '_wandb.'.join(data_config_file.rsplit('.', 1))  # updated config.yaml path
...
def process_wandb_config_ddp_mode(opt):
    with open(check_file(opt.data), errors='ignore') as f:
        data_dict = yaml.safe_load(f)  # config dict
    train_dir, val_dir = None, None
    if isinstance(data_dict['train'], str) and data_dict['train'].startswith(WANDB_ARTIFACT_PREFIX):
        api = wandb.Api()
        train_artifact = api.artifact(remove_prefix(data_dict['train']) + ':' + opt.artifact_alias)
        train_dir = train_artifact.download()
        train_path = Path(train_dir) / 'config/images/'
    ...
    if isinstance(data_dict['val'], str) and data_dict['val'].startswith(WANDB_ARTIFACT_PREFIX):
        ...
        val_path = Path(val_dir) / 'config/images/'
...
class WandbLogger():
    ...
    and basic config metrics and analyses.
    ...
    def log_dataset_artifact(self, data_file, single_cls, project, overwrite_config=False):
        """
        Log the dataset as W&B artifact and return the new config file with W&B links

        arguments:
        data_file (str) -- the .yaml file with information about the dataset like - path, classes etc.
        single_class (boolean)  -- train multi-class config as single-class
        project (str) -- project name. Used to construct the artifact path
        overwrite_config (boolean) -- overwrites the config.yaml file if set to true otherwise creates a new
        ...
        path = (path if overwrite_config else path + '_wandb') + '.yaml'  # updated config.yaml path
    ...
    def create_dataset_table(self, dataset, class_to_id, name='dataset'):
        ...
        arguments:
        dataset (LoadImagesAndLabels) -- instance of LoadImagesAndLabels class used to iterate over the config to build Table
    ...
    def val_one_image(self, pred, predn, path, names, im):
        """
        Log validation config for one image. updates the result Table if validation dataset is uploaded and log bbox media panel
